   ...
   \item To implement a user-level parallel task scheduler: This
    ...
    \begin{enumerate}
        \item will be mapped to a deque (double-ended queue) to which these tasks
            will be pushed into.
        \item will spawn new subtasks that will be pushed into the local deque.
            When the scheduler is ready, tasks will be popped from the same end
            This is the bottom end of the deque.
        \item can have tasks stolen from it by another deque/work-loop but only
            ...
            correctly among possibly several other deques trying to steal tasks
            from this work deque and to measure stealing for different classes of
     ...
     Scheduler deques can steal from each other using different
     strategies. The easiest strategy one can implement is randomly picking up
     a domain other than itself. Another one, suggested in the project problem
     statement, is a round-robin system where each domain attempts to steal
...
reclaimed via a garbage collector; nothing extra need be done for this given
OCaml's GC (garbage-collector). For work-stealing algorithms, where each process
has its own deque and the upper bound for the maximum amount of memory needed by all
deques can be often determined upfront, it can be more memory efficient to keep
all the array buffers in a shared pool. If a process requires a bigger array for
its deque, one is allocated from the pool; the same array can be returned to the
pool when it shrinks to a smaller size. Assuming that the
reclaimed buffers remain in the shared pool of buffers instead of the global
heap, a deque may claim a buffer while there may be still some thieves
referencing it (unlike a GC); this leads to tighter use of allocated memory.
In the interests of finishing the project in time, we did not finish the attempt
to implement this.
...
the internal \texttt{Lin\_domain.Make\_internal} functor and constructing a custom
triple generator using \texttt{QCheck.Gen.triple} that routed owner commands
(Push/Pop) exclusively to the sequential prefix and left domain, and thief
commands (Steal) exclusively to the right domain.

A further subtlety was that the \texttt{cmd} and \texttt{res} types had to be
defined at the top level rather than inside the spec module to avoid OCaml's GADT
type escaping errors. The final test uses \texttt{M.lin\_prop} for
linearizability checking and \texttt{M.stress\_prop} for exception safety,
both driven by the same \texttt{arb\_owner\_thief} generator that enforces the
ownership invariant for deques.
...
A third subtlety was that \texttt{Abort} needed to map to \texttt{None} in the
result representation since STM's sequential model has no concept of a lost race,
and the postcond function needed to accept \texttt{None} as valid whenever the
model predicted either an empty deque or a racing steal. The final test combines
\texttt{STM\_sequential.Make} for
single-domain value correctness, \texttt{STM\_domain.Make} with
\texttt{arb\_triple} for parallel linearizability, and
\texttt{stress\_prop\_par} for crash safety.
...
of the deque, an impossibility. In another instance, while trying to modify some code in the
work-stealing scheduler, it duplicated an \texttt{Array} function which led to a
