...
We think the completing this project worked fairly well given the time
constaint. In terms of helping work out functions, module designs etc., it acted
...
impossibility. In another instance, while trying to modify some code, it
duplicated an $Array$ function which led to a significant debugging effort.
However, in a back-and-forth with Claude, it itself helped pinpoint the error
and helped fix it.

\paragraph{What was surprising.}
The patience with which it would help think through something. This is the first
major project where an LLM has been used to think through and work out a
research paper, so that was surprising. The need for adaptive models is a must
as less powerful models are optimized for quick answers and can come up with
anyting under pressure. At times the unecessary verbosity of something it came
up with was also very surprising, given it should have aware of simpler
alternatives (API functions etc.)
...
We would like to use the metaphor of electric bicycles (bicycles with an
electric battery attached to them). Such bicycles make it very easy to bike up
steep slopes and long distances but without them, one is very limited in one's
skills; the same applies in this case. While Claude was crucial in helping
understand and design the project in a short time, it would be in our
long-term benefit to approach the problem much more slowly, look at existing
...
use it as an easy resource to come up with answers to, in the end, fool oneself
in illusion of really understanding something.
