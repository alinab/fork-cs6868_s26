...
let run_test test_name =
  ...
  | "sequential" | "seq" ->
      ...
      let seq_test = Seq.agree_test ~count:1000 ~name:"FineList sequential" in
  ...
  | "concurrent" | "conc" ->
      ...
      in
      let conc_test =
        QCheck.Test.make ~retries:10 ~count:100 ~name:"FineList concurrent" arb_cmds_par
        @@ fun triple ->
  ...
  | "all" ->
      ...
      let tests = [
        Seq.agree_test ~count:1000 ~name:"FineList sequential";
        QCheck.Test.make ~retries:10 ~count:100 ~name:"FineList concurrent"
          (Dom.arb_triple 15 10 Spec.arb_cmd Spec.arb_cmd Spec.arb_cmd)
